...
<body class="bg-[#E8F1F2] min-h-screen">
    ...
    <header class="bg-[#1B4965] border-b-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div class="max-w-7xl mx-auto px-4 py-4 md:py-6">
            <h1 class="text-2xl md:text-4xl font-black text-white text-center">// EDIT ARTIKEL //</h1>
        </div>
    </header>
            ...
            <form action="proses_edit_artikel.php" method="post" class="space-y-6">
                ...
                <div>
                    ...
                    <select id="label" name="label" required
                        ...
                        <option value="Teknologi" <?php echo ($data['label'] == 'Teknologi') ? 'selected' : ''; ?>>Teknologi</option>
                        <option value="Pendidikan" <?php echo ($data['label'] == 'Pendidikan') ? 'selected' : ''; ?>>Pendidikan</option>
                        <option value="Kesehatan" <?php echo ($data['label'] == 'Kesehatan') ? 'selected' : ''; ?>>Kesehatan</option>
                        <option value="Olahraga" <?php echo ($data['label'] == 'Olahraga') ? 'selected' : ''; ?>>Olahraga</option>
                        <option value="Lainnya" <?php echo ($data['label'] == 'Lainnya') ? 'selected' : ''; ?>>Lainnya</option>
                    </select>
                </div>
                <div>
                    <label for="isi_artikel" class="block text-lg font-bold text-[#1B4965] mb-2">Isi Artikel</label>
                    <textarea id="isi_artikel" 
                              name="isi_artikel" 
                              rows="5" 
                              required
                              class="w-full p-3 border-4 border-black focus:outline-none focus:border-[#5FA8A3] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"><?php echo htmlspecialchars($data['isi_artikel']); ?></textarea>
                </div>
                <div class="flex justify-end space-x-4">
                    <button type="submit"
                            class="bg-[#5FA8A3] text-white px-6 py-3 font-bold border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] hover:shadow-none transition-all">Update</button>
