...
<body class="bg-[#E8F1F2] min-h-screen">
    ...
    <header class="bg-[#1B4965] border-b-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div class="max-w-7xl mx-auto px-4 py-4 md:py-6">
            <h1 class="text-2xl md:text-4xl font-black text-white text-center">// About //</h1>
        </div>
    </header>
    ...
    <div class="flex flex-col lg:flex-row max-w-7xl mx-auto mt-4 md:mt-8 px-4 gap-4 md:gap-6">
        ...
        <main class="w-full lg:w-3/4 bg-white border-4 border-black p-4 md:p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                <h2 class="text-xl md:text-2xl font-black text-[#1B4965]">Tentang Saya</h2>
                <?php if ($_SESSION['role'] === 'Admin') { ?>
                <a href="add_about.php" class="w-full sm:w-auto bg-[#5FA8A3] text-white font-bold py-2 md:py-3 px-4 md:px-6 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] hover:shadow-none transition-all text-center">
                    + Tambah Data
                </a>
                <?php } ?>
            </div>
            ...
            <div class="space-y-4">
                ...
                while ($data = mysqli_fetch_array($query)) {
                    echo "<div class='bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] hover:shadow-none transition-all'>";
                    echo "<p class='mb-4 text-sm md:text-base text-gray-700'>" . htmlspecialchars($data['about']) . "</p>";
                    
                    if ($_SESSION['role'] === 'Admin') {
                        echo "<div class='flex justify-between gap-2'>";
                        echo "<a href='edit_about.php?id_about={$data['id_about']}' class='flex-1 bg-[#5FA8A3] text-white font-bold py-1 md:py-2 px-2 md:px-4 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-center text-sm md:text-base'>Edit</a>";
                        echo "<a href='delete_about.php?id_about={$data['id_about']}' onclick='return confirm(\"Yakin ingin menghapus?\")' class='flex-1 bg-[#FF6B6B] text-white font-bold py-1 md:py-2 px-2 md:px-4 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-center text-sm md:text-base'>Hapus</a>";
                        echo "</div>";
                    }
                    echo "</div></div>";
                }
