...
if (!isset($_SESSION['username']) || $_SESSION['role'] !== 'Admin') {
    $_SESSION['error'] = "Anda tidak memiliki akses untuk mengedit peran user.";
    header('location:beranda_admin.php');
    exit;
}

$username_to_edit = $_GET['username'];
$sql = "SELECT username, role FROM tbl_user WHERE username = '$username_to_edit'";
...
$all_roles = ['Admin', 'Editor', 'Viewer']; // Definisikan semua peran yang tersedia
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-[#E8F1F2] min-h-screen">
    ...
    <header class="bg-[#1B4965] border-b-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div class="max-w-7xl mx-auto px-4 py-4 md:py-6">
            <h1 class="text-2xl md:text-4xl font-black text-white text-center">// EDIT ROLE USER //</h1>
        </div>
    </header>
    ...
    <div class="flex flex-col lg:flex-row max-w-7xl mx-auto mt-4 md:mt-8 px-4 gap-4 md:gap-6">
        ...
        <main class="w-full lg:w-3/4 bg-white border-4 border-black p-4 md:p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <form action="proses_edit_user_role.php" method="post" class="space-y-6">
                <input type="hidden" name="username" value="<?php echo htmlspecialchars($username_to_edit); ?>">
                <div>
                    <label for="username_display" class="block text-lg font-bold text-[#1B4965] mb-2">Username</label>
                    <input type="text" id="username_display" value="<?php echo htmlspecialchars($username_to_edit); ?>" class="w-full p-3 border-4 border-black focus:outline-none bg-gray-100" readonly>
                </div>
                <div>
                    <label for="role" class="block text-lg font-bold text-[#1B4965] mb-2">Pilih Role</label>
                    <select id="role" name="role" required
                        ...
                        <?php foreach ($all_roles as $role_option) { ?>
                            <option value="<?php echo htmlspecialchars($role_option); ?>" <?php echo ($role_option === $current_role) ? 'selected' : ''; ?>>
                ...
                <div class="flex justify-end space-x-4">
                    <button type="submit"
                            class="bg-[#5FA8A3] text-white px-6 py-3 font-bold border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] hover:shadow-none transition-all">Update Role</button>
