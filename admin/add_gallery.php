...
if (!isset($_SESSION['username']) || $_SESSION['role'] !== 'Admin') {
    $_SESSION['error'] = "Anda tidak memiliki akses untuk menambah gambar galeri.";
...
<body class="bg-[#E8F1F2] min-h-screen">
    ...
    <header class="bg-[#1B4965] border-b-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div class="max-w-7xl mx-auto px-4 py-4 md:py-6">
            <h1 class="text-2xl md:text-4xl font-black text-white text-center">// TAMBAH GAMBAR //</h1>
        </div>
    </header>
    ...
    <div class="flex flex-col lg:flex-row max-w-7xl mx-auto mt-4 md:mt-8 px-4 gap-4 md:gap-6">
        ...
        <main class="w-full lg:w-3/4 bg-white border-4 border-black p-4 md:p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <form action="proses_add_gallery.php" method="post" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label for="foto" class="block text-lg font-bold text-[#1B4965] mb-2">Pilih Gambar</label>
                    <input type="file" 
                           id="foto" 
                           name="foto" 
                           accept="image/*" 
